...
**Output file:** `cv/<company>_<role>/KrishnaMaranResume.tex` (create the subfolder first via `mkdir -p`)
**Final deliverable:** `cv/<company>_<role>/KrishnaMaranResume.pdf` — after compilation, delete all non-PDF files from the subfolder. The PDF filename must never contain the company or position name.
...
```bash
cd cv/<company>_<role> && lualatex -interaction=nonstopmode KrishnaMaranResume.tex && rm -f KrishnaMaranResume.tex *.aux *.log *.out *.synctex.gz
```

Expected output: `Output written on KrishnaMaranResume.pdf (2 pages, ...)`. Any page count other than 2 is a failure that must be fixed before presenting to the user.
...
1. Run `lualatex -interaction=nonstopmode KrishnaMaranResume.tex` (from inside `cv/<company>_<role>/`)
