...
**Output file:** `cover_letters/<company>_<role>/KrishnaMaranCoverLetter.tex` (create the subfolder first via `mkdir -p`)
**Final deliverable:** `cover_letters/<company>_<role>/KrishnaMaranCoverLetter.pdf` — after compilation, delete all non-PDF files from the subfolder. The PDF filename must never contain the company or position name.
...
```bash
cd cover_letters/<company>_<role> && xelatex -interaction=nonstopmode KrishnaMaranCoverLetter.tex && rm -f KrishnaMaranCoverLetter.tex *.aux *.log *.out *.synctex.gz
```

Expected output: `Output written on KrishnaMaranCoverLetter.pdf (1 page, ...)`. Any page count other than 1 is a failure that must be fixed before presenting to the user.
...
1. Run `xelatex -interaction=nonstopmode KrishnaMaranCoverLetter.tex`
